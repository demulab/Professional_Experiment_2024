...
: Node("read_write_node")
{
  ...
  set_position_subscriber_ =
    ...
    [this](const SetPosition::SharedPtr msg) -> void
    {
      ...
      uint32_t goal_acceleration = (unsigned int)msg->acceleration;  // Convert int32 -> uint32
      ...
      if (dxl_comm_result != COMM_SUCCESS) {
        ...
        RCLCPP_INFO(this->get_logger(), "\nSet [ID: %d] [Goal Position: %d] [Goal Velocity: %d] [Goal Acceleration: %d]", msg->id, msg->position, msg->velocity, msg->acceleration);
      }
    }
    );
...
int main(int argc, char * argv[])
{
  ...
  setupDynamixel(DXL1_ID);
  setupDynamixel(DXL2_ID);
  setupDynamixel(DXL3_ID);
  setupDynamixel(DXL4_ID);
  setupDynamixel(DXL5_ID);
